...
def create_expense(db: Session, expense: schemas.ExpenseCreate, user_id: int = None) -> models.Expense:
    ...
    # 2. Create General Ledger entries for double-entry bookkeeping
    ...
    update_account_balance(db, account_id=db_expense.debit_account_id, amount=db_expense.amount)
    update_account_balance(db, account_id=db_expense.credit_account_id, amount=-db_expense.amount)
