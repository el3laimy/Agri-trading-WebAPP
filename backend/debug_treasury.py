...
# نحن الآن داخل backend/، لذا app/ موجودة مباشرة
...
def debug_treasury():
    ...
    try:
        ...
        print(f"CASH_ACCOUNT_ID setting value: {cash_id} (Type: {type(cash_id)})")
        if cash_id:
             print(f"CASH_ACCOUNT_ID as int: {int(cash_id)}")
        else:
            print("WARNING: CASH_ACCOUNT_ID is None or Empty! Checking DB table...")
